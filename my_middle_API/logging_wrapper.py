...
def log_and_validate(logger: logging.Logger, validate_output: bool = False, output_model: Optional[Type[BaseModel]] = None):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if validate_output and output_model:
                    ...
                    try:
                        output_model.parse_obj(result)
                        validation_time = time.time() - validation_start
                        logger.info(f"{func_name}: Output validation successful. Time: {validation_time:.4f} seconds")
                    except ValidationError as ve:
                        logger.error(f"{func_name}: Output validation failed: {ve}")
                        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Output validation failed")

                total_time = time.time() - start_time
                logger.info(f"{func_name}: Function executed successfully. Total time: {total_time:.4f} seconds")
                return result

            except HTTPException as http_exc:
                total_time = time.time() - start_time
                logger.error(f"{func_name}: HTTP error: {http_exc.detail}. Status code: {http_exc.status_code}. Total time: {total_time:.4f} seconds")
                raise

            except Exception as e:
                total_time = time.time() - start_time
                logger.exception(f"{func_name}: Unexpected error: {str(e)}. Total time: {total_time:.4f} seconds")
                raise HTTPException(
    ...
    def wrapper(module):
        ...
        for name, obj in inspect.getmembers(module_obj):
            if inspect.isfunction(obj) and obj.__module__ == module_name:
                ...
                new_func = log_and_validate(logger, validate_output=validate_output)(obj)
